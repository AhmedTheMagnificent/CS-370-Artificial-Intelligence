...
write_video = True
debug = True
cam_source = 0  # Replace with your camera source

# Initialize serial communication with Arduino if not in debug mode
if not debug:
    ser = serial.Serial('COM4', 115200)

# Servo motor angle configuration
x_min = 0
x_mid = 75
x_max = 150
...
# Function to check if hand is in a fist position
...
# Function to calculate servo angles based on hand landmarks
def landmark_to_servo_angle(hand_landmarks):
    servo_angle = [x_mid, y_mid, z_mid, claw_open_angle]  # Initialize servo_angle with default values
    WRIST = hand_landmarks.landmark[0]
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                # Write servo angles to Arduino if not in debug mode
                if not debug:
